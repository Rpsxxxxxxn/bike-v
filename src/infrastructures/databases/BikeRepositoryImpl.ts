...
export default class BikeRepositoryImpl implements IBikeRepository {
  ...
  public async get(): Promise<BikeEntity> {
    ...
    const bike: any = await db.get('SELECT * FROM bike', []);
    return BikeEntity.create(bike.id, bike.company, bike.name, bike.model, bike.oil_change, bike.oil_filter, bike.cc);
  }

  public async getAll(): Promise<BikeEntity[]> {
    const db = SQLiteHelper.create();
    const dataList: any = await db.all('SELECT * FROM bike', []);
    return dataList.map((data: any) => BikeEntity.create(data.id, data.company, data.name, data.model, data.oil_change, data.oil_filter, data.cc));
  }
